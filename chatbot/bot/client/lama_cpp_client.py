...
class LamaCppClient:
    ...
    def _load_llm(self) -> Any:
        ...
        try:
            ...
            # Add verbose=False to reduce output, and use_mmap=True for faster loading
            config = self.model_settings.config.copy()
            config.setdefault("verbose", False)
            config.setdefault("use_mmap", True)
            config.setdefault("use_mlock", False)  # Don't lock memory to avoid issues
            
            llm = Llama(model_path=str(self.model_path), **config)
            return llm
        except FileNotFoundError:
            raise
        except Exception as e:
            raise RuntimeError(f"Failed to load model from {self.model_path}: {str(e)}") from e
    ...
    def _auto_download(self) -> None:
        ...
        if not os.path.exists(self.model_path):
            ...
            try:
                ...
                print(f"=> This may take a while. The model file is large (~4-5 GB)...")
                ...
                with open(self.model_path, "wb") as f:
                    if total_size > 0:
                        # Use tqdm with total size if available
                ...
                error_msg = f"=> Download Failed. Error: {e}"
                print(error_msg)
                if os.path.exists(self.model_path):
                    try:
                        os.remove(self.model_path)  # Clean up partial download
                    except:
                        pass
                # Don't raise - let _load_llm handle the missing file
                return
            except Exception as e:
                error_msg = f"=> Download Failed. Error: {e}"
                print(error_msg)
