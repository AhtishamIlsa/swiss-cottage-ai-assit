...
# Try to apply nest_asyncio, but handle cases where it can't patch the loop
# (e.g., when using uvloop in Streamlit)
# This is only needed for async operations, so we make it optional
...
try:
    ...
    # Can't patch the current loop (e.g., uvloop) - this is OK for regular chatbot
    # We'll handle async operations differently if needed
    logger.debug(f"Could not apply nest_asyncio at import time: {e}. This is OK for non-async operations.")
    _nest_asyncio_applied = False


def _ensure_nest_asyncio():
    """Ensure nest_asyncio is applied if needed and possible."""
    ...
    if not _nest_asyncio_applied:
        try:
            ...
            logger.debug(f"Could not apply nest_asyncio: {e}. Using alternative async handling.")
